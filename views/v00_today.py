...
def render(ctx):
    ...
    st.caption(
        "Lưu ý cách đếm: các chỉ tiêu có thể chồng lấn. Ví dụ một nghiệm thu quá hạn "
        "khắc phục có thể đồng thời được tính vào “Mức đỏ”, “Quá hạn” và "
        "“Nghiệm thu còn lỗi”. Vì vậy không cộng các ô chỉ tiêu để đối chiếu với "
        "“Tổng việc cần chú ý”."
    )
    ...
    if actions.empty:
        ...
        display["Mức ưu tiên"] = display["Ưu tiên"].apply(
            lambda x: "Rất cao" if x >= 90 else (
                "Cao" if x >= 80 else (
                    "Trung bình" if x >= 65 else "Theo dõi"
                )
            )
        )
        display = display[
    ...
    st.info(
        "AI ở V0.5.1 làm nhiệm vụ tổng hợp, xếp ưu tiên và cảnh báo theo quy tắc. "
